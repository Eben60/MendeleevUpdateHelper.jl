...
d = @__DIR__
# @show d
function inclpath(fl)
    # println("+++")
    normpath(d, fl)
end

function upd_mend(m_path=nothing; dev = false)
    # println("--")
    # println(inclpath("paths.jl"))
    # println("--")
    include(inclpath("paths.jl"))
    ...
    include(inclpath("PeriodicTable2df.jl"))
    include(inclpath("make_struct.jl"))
    include(inclpath("utype2str.jl"))
    include(inclpath("f_units.jl"))
    include(inclpath("data_import.jl"))
    if ! dev
